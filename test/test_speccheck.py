...
def test_check_hardcoded_path_in_buildroot_tag(package, speccheck):
    """Test if specfile has harcoded path in buildroot tag."""
    output, test = speccheck
    pkg = get_tested_spec_package(package)
    test.check_spec(pkg)
    out = output.print_results(output.results)
    assert 'W: hardcoded-path-in-buildroot-tag' in out


@pytest.mark.parametrize('package', ['spec/SpecCheck2'])
def test_check_hpbt_not_applied(package, speccheck):
    """hpbt: hardcoded-path-in-buildroot-tag
    Test if specfile does not have harcoded path in buildroot tag.
    """
    output, test = speccheck
    pkg = get_tested_spec_package(package)
    test.check_spec(pkg)
    out = output.print_results(output.results)
    assert 'W: hardcoded-path-in-buildroot-tag' not in out
...
def test_check_buildarch_instead_of_exclusivearch_tag(package, speccheck):
    ...
    test.check_spec(pkg)
    out = output.print_results(output.results)
    assert 'E: buildarch-instead-of-exclusivearch-tag' in out


@pytest.mark.parametrize('package', ['spec/SpecCheck2'])
def test_check_biet_not_applied(package, speccheck):
    """biet: buildarch-instead-of-exclusivearch-tag
    Test if specfile does not have BuildArch has any architecture beside noarch.
    """
    output, test = speccheck
    pkg = get_tested_spec_package(package)
    test.check_spec(pkg)
    out = output.print_results(output.results)
    assert 'E: buildarch-instead-of-exclusivearch-tag' not in out
...
def test_check_hardcoded_packager_tag(package, speccheck):
    ...
    test.check_spec(pkg)
    out = output.print_results(output.results)
    assert 'W: hardcoded-packager-tag' in out
...
def test_check_buildprereq_use(package, speccheck):
    ...
    test.check_spec(pkg)
    out = output.print_results(output.results)
    assert 'E: buildprereq-use' in out
...
def test_check_bsisr(package, speccheck):
    ...
    test.check_spec(pkg)
    out = output.print_results(output.results)
    assert 'E: broken-syntax-in-scriptlet-requires' in out
...
def test_check_coid(package, speccheck):
    ...
    test.check_spec(pkg)
    out = output.print_results(output.results)
    assert 'W: comparison-operator-in-deptoken' in out
...
def test_check_coid_is_found_with_double_space(package, speccheck):
    ...
    test.check_spec(pkg)
    out = output.print_results(output.results)
    assert 'W: comparison-operator-in-deptoken' not in out


@pytest.mark.parametrize('package', ['spec/forbidden-controlchar-found'])
def test_check_coid_not_applied(package, speccheck):
    """coid: comparison-operator-in-deptoken
    Test if specfile has no comparison operator in deptoken.
    """
    output, test = speccheck
    pkg = get_tested_spec_package(package)
    test.check_spec(pkg)
...
def test_check_unversioned_explicit_version(package, speccheck):
    ...
    test.check_spec(pkg)
    out = output.print_results(output.results)
    assert 'W: unversioned-explicit-provides' in out


@pytest.mark.parametrize('package', ['spec/SpecCheck2'])
def test_check_uev_not_applied(package, speccheck):
    """uev: unversioned-explicit-version
    Test if specfile does not have Provides: tag."""
    output, test = speccheck
    pkg = get_tested_spec_package(package)
    test.check_spec(pkg)
    out = output.print_results(output.results)
...
def test_check_unversioned_explicit_obsoletes(package, speccheck):
    ...
    test.check_spec(pkg)
    out = output.print_results(output.results)
    assert 'W: unversioned-explicit-obsoletes' in out


@pytest.mark.parametrize('package', ['spec/SpecCheck2'])
def test_check_ueo_not_applied(package, speccheck):
    """ueo: unversioned-explicit-obsoletes
    Test if specfile does not have Obsoletes: tag.
    """
    output, test = speccheck
    pkg = get_tested_spec_package(package)
    test.check_spec(pkg)
    out = output.print_results(output.results)
    assert 'W: unversioned-explicit-obsoletes' not in out
